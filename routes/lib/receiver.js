...
const receiver = {
    ...
    initFindByOnSensor() {
        const _this = this;
        const _query = queryconfig.findBySensor(); // start_time이 Null이 아닌 센서 조회(현재 사용 중인 센서)
    ...
    logInsert(data) {
        ...
        const _this = this;
        // let _query =
        //     queryconfig.logInsert(data);
          let _query =  queryconfig.logInsert(data) + queryconfig.recordUpdate(data);
        //풀에서 컨넥션 획득
        pool.getConnection((err, connection) => {
            if (err) {
                console.log(err);
